...
if __name__ == '__main__':
    ...
    X = hstack([X, coo_matrix((train_match_ct1, (ridx, cidx)), shape=(train.shape[0],1))])
    test_ridx = range(test.shape[0])
    test_cidx = [0] * test.shape[0]
    X_test = hstack([X_test, coo_matrix((test_match_ct1, (test_ridx, test_cidx)), shape=(test.shape[0],1))])
    ...
    # We will use Logistical Regression here..
    ...
    param_grid = {'svd__n_components' : [300, 400, 500, 600, 700, 800],
                  'lr__C': [0.6, 0.8, 1.0, 1.2, 1.4, 1.6, 1.8, 2.0]}
